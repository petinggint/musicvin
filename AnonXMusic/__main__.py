...
async def init():
    if (
        ...
        LOGGER(__name__).error("Assistant Syaaa client variables not defined, exiting...")
        exit()
    await sudo()
    try:
    ...
    await app.stop()
    await userbot.stop()
    LOGGER("syaaaxmusic").info("Stopping berlin x Music Bot...")
